...
class CFlyout
{
	...
	*/
	...
	__New(hParent = 0, asTextToDisplay = 0, bReadOnly = "", bShowInTaskbar = "", iX = "", iY = "", iW = "", iMaxRows = 10, iAnchorAt = -99999, bDrawBelowAnchor = true, sBackground = 0, sFont = 0, sFontColor = 0, sTextAlign = "", bAlwaysOnTop = "", bShowOnCreate = true)
	{
		...
		GUI, Add, Picture, +0x4 AltSubmit X0 Y0 hwndg_hPic vg_vPic, % this.m_sBackground
...
;;;;;;;;;;;;;; Class-specific functionalities, such as WM_LBUTTONDOWN messages, are handled in this function.
...
;Msgbox % st_concat("`n", hWnd, CFlyout.FromHwnd.HasKey(hWnd), Object(CFlyout.FromHwnd[hWnd]).m_hFlyout, WinExist("A"), Object(CFlyout.FromHwnd[WinExist("A")]).m_hFlyout)

	if (CFlyout.FromHwnd.HasKey(hWnd))
		vFlyout := Object(CFlyout.FromHwnd[hWnd])
	; Good workaround but still reserach why hWnd doesn't work...
	else if (CFlyout.FromHwnd.HasKey(WinExist("A")))
