...
;~ }
	...
	MainMenuExist()
	{
		...
		return WinExist("GUI_Flyout1") ; TODO: Chain MainMenu title to CFMH_MainMenu
	}
	...
	/*
		...
		Parameters
			iPos="": if blank, uses MouseY
	*/
	GetRowFromPos(iPos="")
	{
		vFlyouts := Object(this.m_hThread.ahkGetVar["g_aFlyouts", "getPointer"])

		; Get the current wnd under the mouse;
		if (iPos == A_Blank)
			MouseGetPos,, iPos, hMouseWnd

		; See if wnd matches any of our flyout menus...
		for iFlyout, vFlyout in vFlyouts
			if (hMouseWnd = vFlyout.m_hFlyout)
				return vFlyout.Click(iPos, false) ; match, return row under mouse.
